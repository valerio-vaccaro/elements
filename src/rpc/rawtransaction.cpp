...
UniValue combinepsbt(const JSONRPCRequest& request)
{
    ...
    if (base_psbt_index > 0) {
        PartiallySignedTransaction psbt = psbtxs[base_psbt_index];
        psbtxs.insert(psbtxs.begin(), psbt);
        psbtxs.erase(psbtxs.begin() + base_psbt_index + 1);
    }
...
UniValue createpsbt(const JSONRPCRequest& request)
{
                            ...
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                                    ...
                                    {"vout", RPCArg::Type::NUM, RPCArg::Optional::NO, "The output number"},
                                    {"sequence", RPCArg::Type::NUM, /* default */ "depends on the value of the 'replaceable' and 'locktime' arguments", "The sequence number"},
                                    {"is_peg_in", RPCArg::Type::BOOL, RPCArg::Optional::OMITTED, "Whether the input is a peg-in"},
    ...
    for (unsigned int i = 0; i < rawTx.vin.size(); ++i) {
        const UniValue& peg_in_v = find_value(request.params[0].get_array()[i].get_obj(), "is_peg_in");
        if (!peg_in_v.isNull()) {
            if (!peg_in_v.isBool()) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, is_peg_in is not a bool");
            }
            rawTx.vin[i].m_is_pegin = peg_in_v.get_bool();
        }
...
UniValue calculateasset(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2)
        throw std::runtime_error(
            RPCHelpMan{"calculate",
                ...
                RPCResult{
            "{                             (json object)\n"
            "    \"asset_tag\":i <hex>,            (string) Calculated asset tag.\n"
...
UniValue updatepsbtpegin(const JSONRPCRequest& request)
{
    ...
    RPCTypeCheck(request.params, {UniValue::VSTR, UniValue::VNUM, UniValue::VNUM, UniValue::VSTR, UniValue::VSTR, UniValue::VSTR, UniValue::VSTR}, true);
